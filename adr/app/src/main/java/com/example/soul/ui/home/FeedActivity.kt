...
class FeedActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Hi?n th? avatar s?n t? cache n?u có
    ...
    override fun onResume() {
        super.onResume()
        // Ð?m b?o avatar/header c?p nh?t m?i l?n quay l?i Home
    ...
    private fun setupObservers() {
        ...
        viewModel.profile.observe(this) { resource ->
            when (resource) {
                is Resource.Loading -> { /* gi? nguyên avatar hi?n t?i d? tránh nh?p nháy */ }
                is Resource.Success -> {
        ...
        viewModel.sessionExpired.observe(this) { expired ->
            if (expired) {
                Toast.makeText(this, "Phiên dang nh?p dã h?t h?n", Toast.LENGTH_SHORT).show()
                logout()
            }
        }
    }
    ...
    private fun setupListeners() {
        ...
        binding.bottomNavigation.setOnItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.nav_explore -> {
                    Toast.makeText(this, "Tính nang khám phá s? có s?m", Toast.LENGTH_SHORT).show()
                    true
                }
                ...
                R.id.nav_library -> {
                    Toast.makeText(this, "Tính nang thu vi?n s? có s?m", Toast.LENGTH_SHORT).show()
                    true
                }
                R.id.nav_profile -> {
                    navigateToProfileTab()
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    Toast.makeText(this@FeedActivity, "Ðã g?i báo cáo", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(
                        this@FeedActivity,
                ...
                Toast.makeText(this@FeedActivity, e.message ?: "Không th? g?i báo cáo", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun promptOpenSpotifyOrNotify(spotifyUrl: String?) {
        if (!spotifyUrl.isNullOrEmpty()) {
            ...
            Toast.makeText(this, "Không có ngu?n phát", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun openSpotify(spotifyUrl: String) {
        try {
            ...
            try {
                ...
                Toast.makeText(this, "Không th? m? Spotify", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun handleLikeClick(feedItem: FeedItem, isLiked: Boolean) {
        lifecycleScope.launch {
            try {
                ...
                Toast.makeText(this@FeedActivity, "Không th? c?p nh?t lu?t thích", Toast.LENGTH_SHORT).show()
            }
        }
    }
